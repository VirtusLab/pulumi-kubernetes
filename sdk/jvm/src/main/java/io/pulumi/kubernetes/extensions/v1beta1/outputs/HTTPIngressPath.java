// *** WARNING: this file was generated by pulumigen. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package io.pulumi.kubernetes.extensions.v1beta1.outputs;

import javax.annotation.Nullable;
import java.util.Objects;
import java.util.Optional;
import java.util.Map;
import java.util.List;
import java.util.concurrent.CompletableFuture;
import io.pulumi.core.*;
import io.pulumi.core.internal.annotations.*;
import io.pulumi.kubernetes.Utilities;

@OutputCustomType
public final class HTTPIngressPath {
    private final io.pulumi.kubernetes.extensions.v1beta1.outputs.IngressBackend backend;
    private final @Nullable String path;
    private final @Nullable String pathType;

    @OutputCustomType.Constructor({"backend","path","pathType"})
    private HTTPIngressPath(
        io.pulumi.kubernetes.extensions.v1beta1.outputs.IngressBackend backend,
        @Nullable String path,
        @Nullable String pathType) {
        this.backend = Objects.requireNonNull(backend);
        this.path = path;
        this.pathType = pathType;
    }

    public io.pulumi.kubernetes.extensions.v1beta1.outputs.IngressBackend getBackend() {
        return this.backend;
    }
    public Optional<String> getPath() {
        return Optional.ofNullable(this.path);
    }
    public Optional<String> getPathType() {
        return Optional.ofNullable(this.pathType);
    }

    public static Builder builder() {
        return new Builder();
    }

    public static Builder builder(HTTPIngressPath defaults) {
        return new Builder(defaults);
    }

    public static final class Builder {
        private io.pulumi.kubernetes.extensions.v1beta1.outputs.IngressBackend backend;
        private @Nullable String path;
        private @Nullable String pathType;

        public Builder() {
    	      // Empty
        }

        public Builder(HTTPIngressPath defaults) {
    	      Objects.requireNonNull(defaults);
    	      this.backend = defaults.backend;
    	      this.path = defaults.path;
    	      this.pathType = defaults.pathType;
        }

        public Builder setBackend(io.pulumi.kubernetes.extensions.v1beta1.outputs.IngressBackend backend) {
            this.backend = Objects.requireNonNull(backend);
            return this;
        }

        public Builder setPath(@Nullable String path) {
            this.path = path;
            return this;
        }

        public Builder setPathType(@Nullable String pathType) {
            this.pathType = pathType;
            return this;
        }

        public HTTPIngressPath build() {
            return new HTTPIngressPath(backend, path, pathType);
        }
    }
}
